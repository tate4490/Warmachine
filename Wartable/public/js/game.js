// ═══════════════════════════════════════════════════════════════════════════
//  Wartable Local — Game Page
//  Reads state from URL params, connects socket if multiplayer.
//  URL shape:
...
const _initName   = _p.get('name')   || 'Player';
const _initMode   = _p.get('mode')   || 'solo';    // 'solo' | 'multiplayer'
const _initRoom   = _p.get('room')   || null;      // null → create; string → join

// ─── Constants ──────────────────────────────────────────────────────────────
const BOARD_INCHES = 48;
const PX_PER_INCH  = 14;
...
const PLAYER_STROKE = ['#e0e0ff', '#fde68a'];

// ─── App State ──────────────────────────────────────────────────────────────
const state = {
  mode:        _initMode,
  playerName:  _initName,
  playerIndex: 0,
  roomCode:    _initRoom,
  game: {
    round:        1,
...
// ─── Waiting Banner ──────────────────────────────────────────────────────────
function showWaitingBanner(code) {
  document.getElementById('waiting-room-code').textContent = code;
  document.getElementById('waiting-banner').hidden = false;
}
function hideWaitingBanner() {
  document.getElementById('waiting-banner').hidden = true;
}
...
function initSocket() {
  ...
  socket.on('connect', () => {
    if (state.roomCode) {
      ...
      socket.emit('joinRoom', { roomCode: state.roomCode, playerName: state.playerName });
    } else {
      // Create a new room
      socket.emit('createRoom', { playerName: state.playerName });
    }
  });
  ...
  socket.on('roomCreated', ({ code, playerIndex }) => {
    ...
    // Update URL so the back/refresh works cleanly
    history.replaceState({}, '', `?mode=multiplayer&name=${encodeURIComponent(state.playerName)}&room=${code}&player=${playerIndex}`);
    showWaitingBanner(code);
    updateTopBar();
  });
...
function drawGrid() {
  ...
  gridLayer.add(new Konva.Rect({
    ...
    fill: '#1e3a1e', stroke: '#4a7a4a', strokeWidth: 2,
  }));
...
function renderToken(token) {
  ...
  const label = new Konva.Text({
    ...
    fontSize: maxFontSize, fill: '#ffffff',
    align: 'center', verticalAlign: 'middle',
    width: baseRad * 2, height: baseRad * 2,
    offsetX: baseRad, offsetY: baseRad,
    wrap: 'word', fontStyle: 'bold', listening: false,
  });
  group.add(label);
  ...
  group.on('contextmenu', e  => { e.evt.preventDefault(); e.cancelBubble = true; showCtxMenu(e.evt.clientX, e.evt.clientY, token.id); });
  group.on('mouseover',   ()  => { if (!measuring) stage.container().style.cursor = 'grab'; });
  group.on('mouseout',    ()  => { if (!measuring) stage.container().style.cursor = 'default'; });
...
function updateTokenVisuals(token) {
  ...
  if (ring) {
    ...
    ring.stroke(fx.length ? (EFFECT_COLORS[fx[0]] || '#ffffff') : 'transparent');
  }
...
function selectToken(id) {
  if (selectedId && konvaTokens[selectedId]) {
    ...
    const prevBase  = prevGroup.getChildren()[1];
...
function applyRemoteState(remoteState) {
  ...
  for (const id of Object.keys(konvaTokens)) {
    if (!remoteIds.has(id)) { konvaTokens[id].destroy(); delete konvaTokens[id]; }
  }
...
function initPanels() {
  ...
  document.querySelectorAll('.eff-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      token.effects = arr.includes(fx) ? arr.filter(e => e !== fx) : [...arr, fx];
      syncEffectButtons(token.effects);
      updateTokenVisuals(token);
  ...
  document.getElementById('clear-board-btn').addEventListener('click', () => {
    ...
    for (const id of Object.keys(konvaTokens)) { konvaTokens[id].destroy(); delete konvaTokens[id]; }
    state.game.tokens = [];
    tokenLayer.batchDraw();
  ...
  document.getElementById('ctx-select').addEventListener('click', () => { selectToken(ctxTokenId); hideCtxMenu(); });
  ...
  document.getElementById('ctx-remove').addEventListener('click', () => { removeToken(ctxTokenId); hideCtxMenu(); });
...
async function loadForcesList() {
  ...
  if (!forces.length) { section.hidden = true; return; }
...
function guessBaseSize(unit) {
  ...
  if (type.includes('warcaster') || type.includes('warlock')) return 'medium';
  if (unit.base) {
...
function showCardDetail(unit) {
  ...
  detail.innerHTML = `
    ...
    <button class="btn btn-sm btn-secondary" style="margin-top:8px;width:100%"
            onclick="_deployUnit(${JSON.stringify(unit).replace(/"/g,"'")})">
      Deploy to Board
    </button>`;
...
// ─── Dice ────────────────────────────────────────────────────────────────────
function rollDice(diceArray, label) { sendRoll(diceArray, label); }
...
// Expose deployUnit for inline onclick in card detail
window._deployUnit = deployUnit;

function init() {
  // Multiplayer: show waiting banner if we're creating a room, then connect socket
  if (state.mode === 'multiplayer' && !state.roomCode) {
    showWaitingBanner('…');
  }
  ...
  if (state.mode === 'solo') {
    appendChat(null, `Solo mode — welcome, ${state.playerName}!`, true);
  }
}
